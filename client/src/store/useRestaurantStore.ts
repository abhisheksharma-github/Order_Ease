...
export const useRestaurantStore = create<RestaurantState>()(
  persist(
    (set, get) => ({
      ...
      updateMenuInRestaurant: (menu: MenuItem) => {
        set((state) => {
          if (state.restaurant) {
            const updatedMenuList = state.restaurant.menus.map((m) =>
              m._id === menu._id ? menu : m
            );
            return {
              restaurant: {
                ...state.restaurant,
      ...
      setAppliedFilter: (values: string[]) => {
        set((state) => {
          const updatedFilter = [...new Set([...state.appliedFilter, ...values])];
